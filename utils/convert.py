...
def getBaseIPv4(ip, prefix_len):
    toks = ip.split('.')
    base_ip = []

    for i in range(4):
        assert(int(toks[i]) <= 255)
        if prefix_len >= 8:
            base_ip.append(toks[i])
            prefix_len -= 8
        elif prefix_len == 0:
            base_ip.append("0")
        else:
            masked = (int(toks[i]) >> (8-prefix_len)) << (8-prefix_len)
            base_ip.append(str(masked))
            prefix_len = 0

    return base_ip[0] + "." + base_ip[1] + "." + base_ip[2] + "." + base_ip[3]
